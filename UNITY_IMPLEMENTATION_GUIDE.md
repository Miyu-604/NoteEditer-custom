...
譜面の編集対象そのものを表す。  
`Dictionary<NotePosition, NoteObject>` を直接持っているため、Model が表示オブジェクトまで抱えているのが特徴である。

これは一般的な純粋データモデルではなく、Unity 実行時オブジェクトを含むランタイム中心設計である。

## 5.3 EditState / EditorState / Settings / MusicSelector / NoteCanvas
...
毎フレーム `EditData.Notes.Values` を走査し、可視範囲内のノートだけを描画する。
...
- `NotePosition` と BPM/LPB/サンプルの変換仕様
- ロングノートの `prev/next` 接続仕様
